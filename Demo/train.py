...
train_dataset = EnzymeDataset("/scratch0/zx22/zijie/biotranslator/Demo/data/train_70.json")
...
for epoch in range(EPOCH):
    ...
    torch.save(model, f'/scratch0/zx22/zijie/biotranslator/Demo/results/model_ckpt_70/{epoch}_model.pt')
    print("------------------------------------------------------------------")
    print(f"Epoch {epoch}/{EPOCH}, Loss: {total_loss.item():.4f}, Probability: {prob}")
    print("------------------------------------------------------------------")
    

# CUDA_VISIBLE_DEVICES=1 nohup python train.py > log/bio_70.log 2>&1 &