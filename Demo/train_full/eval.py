...
model_path = "/scratch0/zx22/zijie/biotranslator/Demo/train_full/results/model_ckpt_70/9_model.pt"
...
def cal_acc(labels_np, preds_cpu):
    ...
    if np.unique(y_true_np).tolist() == [0, 1]:
        ...
        true_positives = sum(1 for true, pred in zip(y_true_np, y_scores_np) if true == 1 and pred == 1)
        total_true = sum(y_true_np)
        total_predict = sum(y_scores_np)

        accuracy = true_positives / total_true if total_true > 0 else 0
        tp_predict = true_positives / total_predict if total_predict > 0 else 0
        # print(y_true_np, y_scores_np)
        print("acc is", accuracy)
        print("tp_predict", tp_predict)
    else:
...
validation_dataset = EnzymeDataset("/scratch0/zx22/zijie/biotranslator/Demo/train_full/data/halogenase_cut.json")
...
new: AUROC Score: 0.8012226962657291
multi: AUROC Score: 0.7971736988264804
price: AUROC Score: 0.6239745105224137
halogenase: AUROC Score: 0.7056448340232124
"""

"""
10ep:
multi: 0.11475409836065574  0.0875
new: 0.02040816326530612  0.014519056261343012
price: 0.04697986577181208  0.029288702928870293
halogenase: 0.1891891891891892  0.08433734939759036
